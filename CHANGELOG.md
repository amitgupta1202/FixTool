...
- **The eight panel toggles have left the toolbar for tool-window stripes.** Editor, Scenarios, Detail, Connection, Order book, Latency, Terminal and Trace are tabs on the stripe of the edge each one opens from, so the button sits where its window appears and an open window reads as a pressed tab rather than as a grey icon tinted slightly less grey. Each tab names its window and its shortcut on hover, and ⌘1 to ⌘8 (Ctrl off macOS) open them in stripe order. The Trace panel is remembered with the other seven now, so a Ledger left open comes back open. What is left on the toolbar is the workspace, the connect and run controls, the all-sessions actions and the filter, none of which toggles anything.
