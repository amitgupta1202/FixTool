...
- **The demo became an example workspace you keep.** Start Demo Server built three profiles, eleven templates and two scenarios in Kotlin, installed them into **your own** profiles list and scenarios directory, and Stop hunted them back down by id prefix. The FX venue is now plain data in the build, and **Open example…** copies it into a folder you name — default `~/.fixtool/workspaces/fx-venue` — through a dialog asking for a name, a location and the FIX version its sessions speak, where Enter accepts all three. Nothing is mixed into your files, so there is nothing to uninstall: **Close workspace** puts your own profiles and scenarios back and leaves the copy on disk, because it is yours. What you edit in it is yours to keep or commit, and a clean one is another Open away. **Open workspace…** and **Recent workspaces** join the same Quick Connect menu, and the app reopens whichever workspace it was left in.
...
- `POST /demo` and `fixtool_demo` keep working and now take an optional `name` and `fixVersion`: `start` opens the example, `stop` closes the workspace. New `GET`/`POST /workspace` and `fixtool_workspace` report the open workspace, the recent ones and the bundled examples, and open or close one.
