...
- **The eight panel toggles have left the toolbar for tool-window stripes.** Editor, Scenarios, Detail, Connection, Order book, Latency, Terminal and Trace are tabs on the stripe of the edge each one opens from, so the button sits where its window appears and an open window reads as a pressed tab rather than as a grey icon tinted slightly less grey. Each tab names its window and its shortcut on hover, and ⌘1 to ⌘8 (Ctrl off macOS) open them in stripe order. The Trace panel is remembered with the other seven now, so a Ledger left open comes back open. What is left on the toolbar is the workspace, the connect and run controls and the all-sessions actions, none of which toggles anything.
- **The layout button and the three all-sessions view settings have left the toolbar for the pane area's own bar.** They say how every pane draws rather than what the workspace does, so they sit above the panes they change, in one bar that is the same bar in both layouts. The cycling Layout button is three segments now, `Tabs`, split across and split down, because a three-state button that cycles can never show what the next click will do: its tooltip had to read "Layout: Tabs (click for Horizontal Split)", which is a control explaining its own mechanism. Raw and Parsed rows, Hide protocol tags and Group by conversation are named lines with a state mark in **View ▾**, because none of them is pressed often enough to earn a permanent icon and a name beside a tick says more than a glyph that changes shape. On a bar too narrow for them the segments fold into **View ▾** as `Layout ▸` rows and the tabs scroll, so the bar never wraps and **View ▾** is always reachable. Nothing about how the four states are stored has changed.
- **The global filter and the Following chip have left the toolbar for a filter row that cannot go silent.** The regex, the `In` and `Out` boxes and the chip are one query over the panes rather than three app-level buttons, so they share one row directly under the pane bar, above the thing they narrow, in both layouts. The row opens from the funnel at the left of the view controls or with **⌥⌘F** (Ctrl+Alt+F elsewhere), which puts the caret straight in the regex field, and ⌘F is still Search all sessions. It is hidden until it is asked for and **cannot be put away while it is narrowing anything**: the funnel reads as pressed, says why on hover (*narrowing: regex*, *following RFQ 7f3a*), and refuses. The ✕ at the row's right is the way out, and it clears the regex, ticks both directions back on and unfollows before the row goes, so a pane is never left narrowed by a control nobody can see. That was the defect: a regex could sit filtering every session from a field nobody had looked at since they typed in it, and two unticked boxes hid half the traffic without a word. Whether the row is open is remembered with the panel layout, the filter itself is not, and a click on `In` or `Out` now toggles once wherever it lands rather than twice on the tick itself.
