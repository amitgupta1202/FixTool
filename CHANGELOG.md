...
- **Connect, a sessions group, and a run configuration widget where Run ▾ was.** **Quick Connect is now Connect**: every other chip in the row is a verb, the word "quick" was the only thing implying there is a slow way, and the Connection panel is not a slower connect but where a profile is authored. It leads a group that is now only about sessions, **Connect ▾, Disconnect all, Close all**: bring one up, take them all down, close every pane. **Run ▾ has left that group and become a run configuration widget**, the shape an IDE uses: the name of the saved load set or scenario set on the toolbar and a ▶ beside it, so the window always says what happens next. Its ▾ lists every saved configuration and picking one *aims* the button rather than running it, which is what a menu of rows that each started something could never do. A mis-click cost a run instead of a correction, and nothing on screen said which of five sets was next. The choice is remembered in `layout.json` and comes back on the next launch, falling back to the last thing run and then to the first thing saved, so the widget is never a control asking to be configured before it can be used. ⌃R (Ctrl+R off macOS) runs it or stops it, ▶ becomes ■ while its own configuration is running, and a ▶ that cannot run says why on hover. **Load run…** and **Load sets…** are in the same menu and still open their dialogs, and on a workspace with nothing saved the chip reads **Load run…**, because there is nothing to name yet, with the same menu behind it, so Recent and **Load sets…** are reachable from the first launch. On a narrow window the widget gives up its kind, then its name, keeping the ▶ and the name in its tooltip.

- **Two stripes, one dock at the bottom, and one window showing per group.** The bottom stripe is gone. It hung under the window without a frame around it and did not know about the other bottom docks. Terminal, Trace and a new **Documents** tab sit at the foot of the left stripe instead, which is where IntelliJ puts the tabs of its bottom tool windows, and both side stripes now run the full height of the content. Everything that opens across the bottom (the terminal, the Ledger, pinned search results and every open scenario document) is a tab in **one dock**, full width between the stripes, at one height with one tab strip: a load report and a scenario editor land in the same frame as the terminal, and the Ledger finally gets the whole window to draw in rather than the centre column. **A stripe group shows one window at a time**, so a click on another tab replaces it: Editor or Scenarios on the left, one of Detail, Connection, Order book and Latency on the right, one tab in the dock. That is at most three panels, three widths instead of eight, and no state where four right-hand panels squeeze the grid to a strip. The trade is that Editor beside Scenarios, Detail beside Order book, and the Ledger beside a document stop being simultaneous. **The minimize chevrons are gone** from the terminal and the scenario dock, because a stripe tab already hides a dock: hiding is the minimize, and the terminal keeps its shell, and any `claude` running in it, alive while it is away. Selecting a message still opens the detail panel, but only onto a right stripe that is showing nothing, so a click in the grid no longer throws the order book off screen. A layout saved by an earlier build is migrated once on first launch: the first open window in stripe order becomes each group's, and the taller of the terminal's and the scenario dock's remembered heights becomes the dock's.

- **The eight panel toggles have left the toolbar for tool-window stripes.** Editor, Scenarios, Detail, Connection, Order book, Latency, Terminal and Trace are tabs on the stripe of the edge each one opens from, so the button sits where its window appears and an open window reads as a pressed tab rather than as a grey icon tinted slightly less grey. Each tab names its window and its shortcut on hover, and ⌘1 to ⌘8 (Ctrl off macOS) open them in stripe order. The Trace panel is remembered with the other seven now, so a Ledger left open comes back open. What is left on the toolbar is the workspace, the filter, the connect and run controls, the all-sessions actions and the view controls, none of which toggles anything.
- **The toolbar is in groups now, each answering one question, and its buttons say what they are.** Left to right: where am I (the workspace), what am I looking at (the filter), what is connected (Quick Connect, Run, Disconnect all, Close all), what do I do to every session at once (Capture, Search, Blank line), what is dangerous (Clear all, alone), how do the panes draw (the layout segments and View ▾), and Settings and Help where an application has always kept them, with a 1dp rule between each group and the next. Capture, Search, Blank line and Clear all are labelled chips of the same kind as Run and Disconnect all rather than unlabelled glyphs, and Clear all wears the warn colour, because the one control in the row that destroys something should not look like the one beside it that adds a blank line. **The global filter is the middle of the row**: the regex, the `In` and `Out` boxes and the Following chip, centred, taking the width the groups either side do not want. **The layout and the three all-sessions view settings are at the right**: three segments for tabs, split across and split down, because a three-state button that cycles can never show what the next click will do, and Raw or Parsed rows, Hide protocol tags and Group by conversation as named lines with a state mark under **View ▾**. On a window too narrow for all of it the chips drop their words for their glyphs and say them on hover, then the segments fold into **View ▾** as `Layout ▸` rows, then the filter shrinks: the row never wraps to a second line, and the workspace, Settings and Help never fold. Nothing about how any of these settings is stored has changed.
- **The pane bar and the filter row are gone, and the panes have their rows back.** One sat above the panes in each layout and one under it, and both were right about where a control belongs and wrong about what it costs: two rows of the scarcest thing in the window in the split layouts and one in tabs, to hold six controls, while the middle of the toolbar sat empty. The rule the filter row was built for holds without it, and more plainly: the filter is on the toolbar at all times, so a regex narrowing every pane is one nobody has to open anything to see. The funnel that opened the row and its **⌥⌘F** are gone with it, and ⌘F is still Search all sessions. The tab strip keeps the scrolling it gained, so fifty lanes' worth of tabs still cannot push a pane's own buttons off the right edge.

### 🐛 Fixed

- **The run configuration widget follows the workspace it is in.** Reported from a desk: a workspace with load sets saved in it was opened and the chip went on reading **Load run…**, the empty state the window before it had come up in. The widget read the saved load sets, the saved scenario sets and the run records straight off disk inside a `remember` keyed on the things it could see change, its own menu opening, a run starting, the load set editor closing, and not one of those moves when a workspace re-points the stores at another directory. The same blind spot hid a set saved from the rail or written while the menu was shut. Those four lists are the ViewModel's state now, refreshed by every door that writes one: a set saved or deleted, a run started or finished, a workspace opened, closed or reset. The chip's name, the kind beside it, the ▶ and the reason a dark ▶ gives all follow it with nothing clicked, and the rule that picks a default when nothing has been chosen reads the same lists the menu is drawn from rather than going back to disk on its own.
...
with every number in it, and then asked its reader to bring three things from outside the window:
which sessions a set needed, connected by hand before it would start; what `peak outstanding` and
`strays` meant, defined nowhere in the product; and what value a gridline sat on, in two charts that
drew their grids and labelled none of them. A run opens its own doors now — every profile its live
phases issue from or listen on is connected before phase 1 dials, acceptors first, and only where the
run would have been refused — and the Run menu says which sessions a set needs and which of them
pressing it will connect. Every figure the report prints carries its meaning on hover, keyed by the
tag rather than the label so the same number reads correctly when it becomes "late to phase 2" inside
a set. Both charts label the decades and the guides they draw, on round numbers a reader thinks in,
and each axis says what it measures. The guide gained the two reference tables that were missing
under all of it: every field a run asks for, and every figure its report prints. And Close all, next
to Disconnect all, puts a box that has just run a fifty-lane set back to nothing. This one is also a
release you can install: v1.18.0 was tagged and never got past its own test job, so everything in it
arrives here.

### ✨ Added

#### A run brings up what it names, and Close all puts the box back

- **A load run connects the profiles it names, instead of refusing over them.** Which sessions a saved set needs is the one thing its name does not say, so running one meant opening the file, connecting its profiles by hand, and finding out one refusal at a time which one you had missed — and a two-sided set is two of those before a single message goes out. `fixtool load --set` never had that problem: its host opens every lane the set names, waits for logon and reports whatever did not arrive. The window does the same now. Every profile a live phase issues from or listens on that has no session logged on is connected before phase 1 dials, **acceptors first** so a port is bound before anything dials it, and the run waits up to ten seconds — the same ten a fan-out lane gets and the scenario runner's own auto-connect gets — for the lanes to reach LOGGED_ON. It fires **only where the run would have been refused**: a profile already up is left exactly as it is, and one nothing could fix (a name no saved profile answers to, an acceptor asked to issue) is still refused before anything dials. A venue that never answers is the refusal it always was, *no session of 'LOADGEN' reached LOGGED_ON, so there is nothing to issue on*, said after the wait rather than before it. **And the venue with them, when the venue is one of ours**: a set names the profile that issues and never the endpoint it dials, which is right when that endpoint is somebody else's server and wrong for the bundled examples, where the lanes dial a FixTool acceptor on loopback — bringing up the client alone left five lanes dialling a port nothing had bound. The lanes are left up afterwards, as an auto-connected scenario session is: the run borrowed them, it did not own them. The claim covers the panes the run is **about to open** as well as the ones already up, so a scenario cannot start on a lane between the moment the run dials it and the moment it logs on. `POST /load` answers 202 where it used to answer 409, and the Run menu's load rows are no longer greyed out with nothing connected — a greyed row would be hiding the door that connects them.
- **The Run menu says which sessions a set runs on, and which of them pressing it will connect.** *Load set ▸ RFQ round trip 3 phases* said how much there was and nothing at all about what it needed. Under it now: *on LoadGen, RFQVenue · Run connects RFQVenue*. A parked phase's profile is nobody's business and is left out; a name no saved profile answers to is listed as needed all the same but never as something Run will connect, because the set's own refusal is what names that, and it opens the editor on the set.
- **Close all, beside Disconnect all.** The half Disconnect all deliberately leaves undone: it closes every session pane, disconnecting them on the way so the far end gets a Logout rather than a socket that stopped answering. A load set that opens fifty lanes leaves fifty panes, and putting the box back to nothing meant closing them one at a time. It counts **panes**, not connections, because a pane left over from a run you have already disconnected is exactly what it is for. **This one asks**, where Disconnect all does not, and it asks in the button rather than in a dialog: the first click arms it — *Close 50 panes? Click again* — the second closes them, and it disarms itself after five seconds so an armed button never sits waiting for somebody who has forgotten what it is armed for. It asks because the difference between the two buttons is exactly what cannot be put back: Quick Connect returns the sessions, and nothing returns a pane's messages. Profiles, workspace, scenarios, run records and load reports are untouched, and a live load run or set refuses it in the sentence Disconnect all is refused with. `POST /sessions/close` and `fixtool_close_sessions` are the same action for a script, reporting how many panes closed and over how many profiles, and `sessions: 0` rather than an error when none was open.

#### The report explains its own figures, and the charts name their scales

- **Every figure in the report carries its meaning on hover.** The report had no tooltips at all: a figure was a label and a number, so `peak outstanding`, `drain`, `duplicates` and `strays` were defined nowhere in the product and somebody reading a finished run had to go and find the guide, or read the source. That is a poor trade for a number whose whole job is to be read once, in passing, by somebody deciding whether a run proved anything. The two figures that decide the verdict, the seven of the quiet strip, the four of the tool's own part and every per-lane column now say what they mean. Definitions are keyed by **test tag rather than by label**, because a label moves and a tag does not: the same figure reads *unanswered* when a run is over and *outstanding* while it is going, and *late* becomes *late to phase 2* inside a set. They go on labels and column headers, **never on cells** — a lane table is one header row and up to fifty rows of six, so one per cell would be three hundred hover targets that grow with the run against six that do not, and `LoadRunDocumentTest` renders the same report at two lanes and at fifty and holds the two counts of explained nodes equal, which fails the moment somebody puts one on a cell. The bubble is anchored to the label rather than to the number, so it holds open while the report ticks every 250ms underneath it, and the meaning is on the label's own semantics node as well as in the bubble, so a test can assert it and a screen reader gets it for free.
- **Both charts label the gridlines they draw.** The ranges lived in a key underneath, so a reader could have the four corners of a plot and no value inside it — and on the outstanding curve, which is logarithmic on both axes, that is worse than it sounds, because the midpoint of a decade is 3.16 and not 5, so an unlabelled log grid says less than no grid at all. Every decade is named now, as the percentage and the duration it is; the verticals moved onto round numbers to make that worth reading, off a floor of 500µs that had them sitting on 5ms and 50ms; **p50, p95 and p99 are named at their own guides**, where they were three identical lines told apart by matching left-to-right order against the key; and the x axis says that it plots the share still unanswered against elapsed time, which is the whole of what makes it the burst's wait-time chart. The per-second panel gains both its scales and, at last, a time axis, on steps a reader counts in — 41 seconds over five labels is 8.2, so the step rounds to 9 — and a tenth of headroom above the tallest thing, because the top of the plot used to be the taller of the tallest bar and the schedule, so a run that held its rate welded the schedule line to the frame. What the keys said about ranges is gone, because the plots say it themselves; what is left in them is what a mark cannot carry: what red means, that the axes are logarithmic, and whether a column is still one second or has become a range of them.

### 📖 Documentation

- **Every field a run asks for, and every figure its report prints.** The load chapter explained the ideas and defined almost none of the numbers: `peak outstanding`, `drain`, `duplicates`, `never left socket` and `issue failures` appeared nowhere in it. Two reference sections, both as **tables**, because a definition somebody looks up one row of is not prose. What a run asks you for: every field of the dialog, what it decides, and the flag that answers it headless — including the two that decide whether the numbers mean anything at all, the settle window and the store. And what the report says: the verdict and the two figures it is about, the quiet strip that explains them, the round trip and what it is measured between, the tool's own contribution, and the three verdicts with the exit code they add up to. The facts worth getting wrong are pinned by `HelpDocTest` as the other chapters are — that unanswered is the bar and duplicates are never judged, that a late reply answers nothing, that a stray is somebody else's traffic, that issued is three numbers and completeness is judged over the last of them, and that **exit 2 means the run never started**, which a build that cannot tell it from a venue failing will chase in the wrong place.
- **The README says what a load run and a load set are.** It had no mention of either. Its "Multi-Session Load Testing" bullet was about opening many sessions — a different thing wearing the same word — and is now named for what it is.

### 📦 Distribution

- **The release build produces installers again, and v1.18.0's changes ship inside this one.** v1.18.0 was tagged and pushed and never released: its test job failed two tests three attempts each with `java.awt.HeadlessException`, the three packaging jobs never started, and the newest thing anybody could install stayed **v1.17.0**. Two causes, wanting different answers. *Copy as `fixtool load --set`* reached the system clipboard unguarded, so on a runner with no clipboard the exception came back out of the click handler and failed a test about whether a set is saved before its name goes out — a copy is a convenience and a box with no clipboard has nothing to copy to, so it is swallowed where it cannot happen, as the record's own copy already was. The other is not carelessness: the load sets editor is a real AWT window, which cannot be constructed with no screen at all, so **the test job runs under a virtual display**. Skipping the test on CI was the alternative, and it would have meant the release build never checking a path a user clicks while reporting a pass. Nothing was lost in the meantime — 1.18.0's load sets, captures, reactive phases and quote book are all in this release, so anybody coming from 1.17.0 gets both.

### 🐛 Fixed

- **Close all no longer leaves behind the panes a venue was about to open.** A venue's arrivals are hopped onto the view model's own scope before they become panes, so a handful of them can be queued behind the close that takes the venue and its clients away together. They landed afterwards and minted a pane per client for a venue that had none — panes showing a conversation on an engine that had stopped, which is the orphan `attachVenueClient` already declines to open for a venue whose *profile* has gone. A venue whose *pane* has gone is the same orphan by another route, and is declined the same way. Found by pressing the button: eleven panes went, and five came back a moment later.
- **A muted phase's profile no longer refuses the set that parks it.** Muting a phase promises that no lane of its profile is opened and nothing about it is judged, which is what "the venue leg this phase issues on is down" needs from parking it — and `LoadSetRunner` has always honoured it. The window and the control surface gathered lanes for **every** phase before starting, muted ones included, so a three-phase set with its middle phase parked on a dead profile was refused before it started, over a phase it was never going to run. Both start paths now gather for live phases only, as the runner does.
- **A phase row breaks between its clauses, never inside one.** The set band draws each phase's sentence in one wrapping line, and a capped reactive phase is the longest sentence a spec can print: it overran and broke between "after phase" and "1", so the second line opened with "1 · settle 1m", which reads as a phase ordinal to anyone scanning the band rather than reading the sentence. Every clause of `describe()` that names a number is written on no-break spaces now, so a row can only ever break at a "·" — gluing the ordinal alone was tried first and moved the break one word left onto "after" / "phase 1", which is the same misreading one word cheaper. The plans a set prints over `GET /load-sets` carry it as well, because the sentence an agent reads is the sentence a person reads. No assertion could have caught the original: the semantics node holds one string and knows nothing about where the line breaker put its hands.
- **The reactive ceiling's field is drawn off while its tick is clear.** The Speed row left the field editable whether the *never above* box was ticked or not, so typing 999 into it with the tick clear said three things at once — a field holding a ceiling, a hint under it reading "Unticked: as fast as they arrive", and a footer saying "reactive after phase 1" with no cap in it — and the number was taken, kept until Done and then dropped, with nothing anywhere saying so. The tick is the one decider, so the field it decides is drawn off: sunken fill, grey text, no caret, and disabled in the semantics so a reader that never sees the fill is told the same thing. The value stays rather than being cleared, because a ceiling a ticked box put there is worth reading back if the box goes on again, and greyed it cannot be mistaken for one the run will honour. Typing does not tick the box — that would hide the decider inside the field it decides. `SlimField` grows the disabled state it never had, on the recipe `SlimButton` already wears, and every other caller keeps the default.
- **The Reacts to hint opens like the sentence it is.** Every other lowercase hint in the phase editor is a grammatical continuation of the control above it — "1m" reads on into "after the last send", "500" into "a second, for". This one is not: it is its own sentence about what the picker above it means, and it opened lowercase one row above "A ceiling, not a schedule", which is the same kind of sentence opening the other way.
...
#### One Run menu, and Disconnect all

- **Run ▾ in the toolbar is the run-configurations chooser.** The only Run menu in the app was inside the Scenarios rail's header, because scenarios were the first thing that could be run and everything runnable since was added to the same menu: run sets, fan-out, the load run, and then load sets. So somebody who wanted to run *RFQ round trip* opened the message editor, found a Load button, and got a form for one message. A load set is not a scenario: it has no steps, no expectations and no bindings, and its only connection to the rail was that the menu was already there. **Run ▾** sits beside Quick Connect, where every other app-level action already lives, and holds one row per saved load set and per saved scenario set, each running at once, then **Load run…**, **Load sets…**, and **Recent** as its own titled group with the verdict first. A menu about saved things is also where you find what they produced.
- **The rail's menu keeps only what reads the scenario list.** Thirteen items of four kinds became seven of one: Run ★ favourites, Run filtered, Run selected, Repeat a scenario ×N, Run examples table, Fan out over sessions, and **Save these as a set…**, which names the selection it saves and is how a scenario set gets into the toolbar's Run ▾. Every item left reads the list beside it: what is starred, what the filter shows, what is ticked, which scenario has a table. The editor's Load button does not move, because it runs the message in front of you. Nothing is removed from the app, and every moved item keeps its test tag.
- **Disconnect all, in the toolbar and on the control surface.** `disconnectAllSessions()` existed and ran only at shutdown and on workspace close, so putting a box back to nothing connected was one profile at a time. The button beside Quick Connect takes the count of sessions it will drop in its tooltip, and **it asks no confirmation**, because a disconnect loses nothing: reconnecting is one click and the books, the records and the panes all survive one, so the venue's orders and quotes stay where Clear order book can reach them. The one case that would lose something is a live load run or set, and there the button is disabled and says why. `POST /disconnect {"all": true}` is the same action for a script, answering how many sessions went down and over how many profiles, `sessions: 0` rather than an error when nothing was up, and **409 while a load run or set is live**, in the sentence the tooltip carries. `fixtool_disconnect` takes the same optional `all`.

#### Load sets: several runs in order, under one seed, with one report

- **A load set is several load runs in order, as a saved file.** Most load proofs against a venue are two or three runs that depend on each other. Burst NewOrderSingle, then cancel the same ClOrdIDs. Ask for quotes, then order against them. That was a wrapper script around `fixtool load`, and the script owned the seed, the stop rule and the combined report, which are the three things the tool already does well for one run. `<workspace>/load-sets/<name>.json` is an ordered list of **phases**, each a complete load plan with its own template, profile, listen profiles, match tags, shape and settle window, plus the two things the set owns: the **seed**, because sharing it is the whole point, and the **store and log**, because applying them per phase would put a logon and a sequence reset in the middle of the set's clock. `fixtool load --set rfq-round-trip --seed run=$BUILD_NUMBER --junit reports/rfq.xml` runs one, and `POST /load {"set": "rfq-round-trip"}` starts the same job over the control surface, with `GET /load-sets` listing what is saved.
- **A seed value may be a generator, rendered once when the set starts.** A saved set with `"run": "b7f2"` would give every execution the same ids and the venue would see duplicate ClOrdIDs on the second night. `"run": "${uuid:4}"` is rendered once through the renderer the templates already use and then frozen, so the file says `${uuid:4}` and the record says `b7f2`. `--seed run=…` and the API's `seed` override it, which is how a build makes its own number the run id so the venue's logs and the build log agree.
- **A phase can start counting where another left off.** `${messageIndex}` is 1-based per phase, so there was no way to say "the other 2,000". `"indexFrom": 2001` on a phase adds one integer to the pacer's counter, which is what makes a three-phase RFQ set possible: ask for 4,000 quotes, hit the first 2,000, pass the other 2,000. It appears in the phase heading as "×2,000 from 2,001" and in the record.
- **Nothing dials until every phase is fine.** Every phase goes through the same refusals a single run does, each sentence carrying its phase: "Phase 2 · Hit the first 2,000: The template reads `${desk}` and nothing seeds it. Pass `--seed desk=…` on the command line." The store's own sentence is printed once for the set rather than once per phase. A set of six must never fail on phase five for something that could have been said before phase one.
- **A reply that arrives during a later phase is credited to the phase that asked for it.** The set owns one stamp listener per session. Every reply is offered to the live phase first, then to each finished phase newest first, and only a reply nobody issued is a stray, on the phase that was running when it came. Against a matching venue this is not a corner case: phase 1's orders keep drawing fills while phase 2 cancels them, and one matcher per run counted every one of those fills as a phase 2 stray, which is the number the "nothing matched" diagnosis reads.
- **A phase that did not pass stops the set, and the rest say so.** A phase passes when its exit code is 0, which is the rule `fixtool load` already exits on, so a set and a wrapper script agree on what "failed" means. `--on-failure stop` (the default) reports the remaining phases as SKIPPED, each carrying its plan and a note saying why, so a reader can still say what the set was for. `--on-failure continue` runs them all and the set verdict names the first that did not pass. Stopping by hand stops the running phase and skips the rest. Either way the set exits 1, because a build cannot pass on a run somebody ended.
- **One record, one JUnit file.** The set writes `loads/<id>/load.json` after every phase and every 250 ms inside one, so a set killed at phase three leaves phases one and two complete on disk and the morning after an overnight set the answer is in the file whether or not the app is still open. `--junit` writes one `<testsuites>` with a `<testsuite>` per phase, and a skipped phase is three `<skipped/>` cases carrying its note, which a build server draws as grey rather than as missing. A one-phase run keeps writing the bare `<testsuite>` it wrote before, mirroring `fixtool run` for one scenario and for a set, so nobody's 1.17 pipeline changes shape.

- **A phase can be muted.** A three-phase set is authored once and run for weeks, and for stretches of that time one of its phases is in the way: the venue leg it issues on is down, or the failure being chased is in phase 2 and phase 3's sixty-second settle window is noise. The only way to run the set without it was to delete the phase and author it back, or to keep a second copy of the file that drifts from the first. The *mute* chip on a phase row parks it, and the set skips it on every run: it keeps its template, its profile, its shape and its place in the order, no lane of its profile is opened, and nothing about how it would have behaved is judged, so a parked phase that reads a name nothing seeds blocks nothing. **Any** phase, not only the last, because position is a proxy for dependency and the set already knows the real dependency graph: the one case the tool can prove is broken, a later phase reading a name only a muted phase keeps, is refused before a lane dials, in the reading phase's voice and naming the muted phase by number and label. The report carries it as a **skipped** phase noted `muted in the set`, at its own number, with `⊘` in the rail, `MUTED` on the badge and in the terminal's status column, and the verdict counts it apart, so a set that passed on the two phases that ran reads *2 passed, 1 muted* rather than *1 skipped*. `"muted": true` is written only on the phase that is, the set file's schema stays 1, and because the flag is in the file `fixtool load --set` in a build honours it and `GET /load-sets` says how many phases a set will skip before an agent starts one.
- **An agent can discover the saved sets and read one without being told its name.** `GET /load-sets` and `GET /load-sets/<name>` were HTTP only, so an agent driving FixTool over MCP could run a set with `fixtool_load set=…` but had no way to find out that *RFQ round trip* existed, or what its phases did. `fixtool_load_sets` is those two reads behind one tool: with no argument it lists what is saved as `{count, sets:[{name, label, phases, profiles}]}`, and `name=<set>` returns that set whole, the same JSON the file holds, down to each phase's template, match, shape and captures. MCP has no status codes, so a name nothing answers to is the route's own sentence in the body rather than a 404. Read-only, because a set is authored in the app or in a checkout.

#### Captured values: a set against a venue that mints its own ids

- **A phase keeps named tag values off each matched reply, and a later phase addresses them.** A shared seed only works against a venue built to be addressable from it, which is what the bundled RFQ venue was: `131=RFQ-7` was quoted as `117=Q-RFQ-7`, so phase 2 could name a quote it never saw. A real counterparty mints an opaque QuoteID and prices the quote itself. A phase's `"capture": {"quoteId": 117, "offer": 133}` keeps those values at each request's own message index, and a later phase reads them as per-message names: `117=${quoteId}`, `44=${offer}`, resolved by the same `${messageIndex}` its own ids are built from. That is what lets a load set run against something other than FixTool's own acceptor.
- **Only an earlier phase's captures are readable, and the refusal says so.** "Phase 2 · Hit the first 2,000: the template reads `${quoteId}` and no earlier phase captures it. Add a capture to a phase before it, or seed it." A capture name has to be a name a template can read, and it cannot shadow a seed, a lane's own name, `messageIndex`, or another phase's capture, because a later phase's message must not depend on which of two things the renderer reached first.
- **A message that could not be addressed fails the phase, by index and by name.** The venue answering one QuoteRequest without a 117 means the next phase has nothing to hit with. That message is never sent rather than sent with a literal `${quoteId}` in it, and the report says `INCOMPLETE  1 of 20 not sent: no quoteId for index 7` and exits 1. Its own word rather than "unanswered", because "the venue did not reply" and "the tool never asked" are different findings. The pacer keeps its schedule either way, and a rate run's short second is reported as the shortfall it is. The document draws the missing messages as a table beside the unanswered one, and the phase editor's Advanced group gains a Capture row.
- **The values themselves are evidence.** `NN-captured.tsv` beside each phase's other files, one tab-separated line per index, so "what did the venue say for 412" is a grep rather than a parser. The record's phase carries `capture: {names, captured}`, and the counts are shown per name whenever they differ, because that difference is the venue answering without a tag and is exactly what the next phase is about to trip over.

#### Reactive phases: a set of blocks becomes a set of chains

- **A phase can react to the one before it, issuing one message for each reply that phase is given, as the reply lands.** A three-phase RFQ set asked for a burst of 200 sent 200 QuoteRequests, waited out phase 1's whole settle window, sent 200 Quotes, waited again, then sent 200 QuoteResponses. Three blocks, and three legs measured separately, because nothing in a staged set is one journey. A **reactive** phase turns the same file into 200 chains that each run at the venue's own speed. **It does not make the set faster**: with four of 200 QuoteRequests unanswered, phase 1's settle window still runs its full 60 seconds. What it changes is that the 196 chains that worked finish in the first 200ms instead of a minute later, and that the report can say what one whole journey cost. `"shape": {"kind": "triggered"}` with `"after": 1` beside it in the set file, `Reactive` on the Shape segment from phase 2 on, and `fixtool load --set` runs one with no flag of its own, because the shape is in the file and there is no count to pass.
- **Its count and its message indices are its trigger's, both.** A reactive phase asks for no number and has no *from N*: the message it answers is at its trigger's own index, which is what keeps `${messageIndex}` lining up along the whole chain. So **phase 1 can never be reactive**, and the volume of a chain is set once by the phase that starts it and inherited from there. It reads only what its trigger kept and what its trigger could itself read, never a sibling's captures: with two phases both reacting to phase 1, phase 1's match fires them at the same moment, so neither has answered for the other yet, and reading across is a race rather than an untidiness. A message whose trigger never fired is **unaddressable**, not sent, counted by index, and the phase is INCOMPLETE, exactly as a missing capture is.
- **The chain block: what one message cost all the way through.** One line per leg, with the round trip each leg took and the handover between them, so "which hop got slower" is answerable from the report rather than from three reports side by side. The document draws it under the phase that ends the chain, and the terminal prints it in a build log.
- **A cap is a ceiling, never a schedule.** A reactive phase can be held to a number of messages a second, aggregate across every lane, and sitting under it is the normal, healthy case: it means triggers arrived more slowly than the ceiling, which is the venue's answer and not the run's failure. Reusing the rate verdict for it would report **every healthy reactive run as red**, so a capped phase gets its own verdict, **CAPPED**, the chart draws the ceiling and paints no second red for being under it, and Compare refuses to rank one capped run against another. In the dialog it is a *never above* tick beside the field, and the row says the ceiling is across all the lanes.
- **The set says which phases are going, not which phase is on.** Phases can now overlap, so everything that named "the running phase" had to learn to name several: the rail, the terminal's narration, the poll row and the set verdict. A phase waiting for its trigger says it is waiting rather than reading as pending forever, a phase whose trigger threw is told so by name, and what the panes discarded belongs to the set rather than being counted once per overlapping phase. `GET /load-sets` gains a plan line per phase, because a phase count reads the same for three blocks and for a chain of three.

#### A venue that remembers what it quoted

- **A quote book, beside the order book.** The rules engine could read what the venue was holding in orders and nothing at all about what it had quoted, so a hit on a live quote, a hit that arrived too late, a second hit on a quote already answered, and a hit on a quote this venue never sent were the same message to a rule, and a venue that answers all four the same way is a message echo, not something a client can be tested against. The quote book is fed from the wire exactly as the order book is, with one thing reversed: a quote is **born by a message the venue sends** and answered by one it receives, which is why it is a second small book rather than a setting on the first. It keeps the id, the instrument, both prices and their sizes, and the ValidUntilTime, per counterparty, under the same cap and the same DONE-first eviction. **Clear order book** empties both, because they are one venue's memory.
- **`whenQuote` on a trigger: `unknown`, `open`, `expired`, `done`.** `whenOrder`'s sibling, ANDed the same way, read **before** the incoming message is recorded so a hit is judged against the quote as it stood when the hit arrived. `expired` is a clock comparison made when a rule asks rather than a state anything writes, because a quote does not go stale at a moment anybody sends a message. `GET /acceptor/rules` reports it beside `whenOrder`, and the rule card reads *and the quote is expired*.
- **`quoteField`: a condition that compares a tag against the quote's own value.** *"The client hit the price we quoted"* was inexpressible. No literal can say it, because every quote carries a different price, and no tag on the incoming message can either: the price the client sends is precisely the claim under test. `{"tag": 44, "matcher": {"type": "quoteField", "name": "offer"}}` resolves against the venue's own record before the condition is judged. It is `reference`'s mirror image and each is refused by name on the other's side of the tool: a reference has no scenario scope in a trigger, a quote field has no venue book in a scenario. With nothing to resolve against it is **false**, which is what lets the unknown-quote rule sit below the hit rule and answer instead.
- **`${quote.<name>}` in a reply, over the same nine names.** `31=${quote.offer}` books a hit at the price the venue actually quoted rather than the price the client claimed. Refused rather than substituted empty, and the refusal matters more here than for orders: the field is a *price*, so an empty substitution would put `31=` in a trade confirmation and the client would be blamed for it. So a rule whose reply reads `${quote.…}` will not validate without a `whenQuote` of `open`, `expired` or `done`. Unlike an order, no trigger can mint the quote it reads, because a rule never answers the venue's own send.
- **A dry run takes the quote too.** `POST /acceptor/test` gains `quoteState` and a `quote` object of the venue's own names, because a quote rule asks both questions at once: *is it open, and what was its offer*. "What would this venue do with a hit that arrived too late" is now answerable without waiting thirty seconds for one. It reports `assumedQuoteState` back and, per rule, `whenQuote` with what it asked for, what it read and the verdict. The condition it reports for a `quoteField` is the comparison actually made, so a reader sees `exact 1.09013` rather than a question.
- **An ExecutionReport that books a quote hit no longer reaches the order book.** It carries a ClOrdID the venue never saw on an order, so the order book could only file it as an unattributed report. That was noise at the bottom of every RFQ run.

#### Two generators, so a venue can price its own quotes at load

- **`${random:<min>:<max>:<decimals>}`** draws a number from the closed range and quantises it, in `BigDecimal` throughout so the wire carries `1.09013` and never `1.0901300000000001`. It is what replaced the RFQ venue's frozen price, and it is rendered natively: a burst of four thousand quotes costs microseconds where the same field as a Kotlin expression cost about 60 ms each on the one dispatch thread every acceptor reply shares.
- **A seconds offset, `${utcnow+30s}`.** The finest unit was the minute, so a quote's thirty-second validity could not be written. `min` still means minutes and a bare `m` still means months.

### ⚡ Performance

- **Every message the app parses is 1.4x to 1.9x cheaper, because one question is now asked before the work rather than after it.** `FixMessageHelper.toQuickFixMessageManual` runs on every message QuickFIX/J hands up, which on a load run is every reply, on the engine's own callback thread. It has to know which repeating groups the dictionary does *not* define, because a flat parse collapses those to their last instance and that is data loss rather than a rendering blemish. It asked by building a whole `GroupOverlay`, the dictionary traversal and the period-detection guess and a label per entry with a dictionary lookup per row, then threw away everything except one yes-or-no that is "no" for every conformant message. `GroupOverlay.salvageable` answers it directly: a rescue needs a count row whose run no dictionary-defined group already covers, so when every count row on the message is a defined group whose delimiter follows it, there is provably nothing to rescue and neither walk is made. Anything the test cannot prove takes the path it always took. A 14-field ExecutionReport went from **5,552 B and 38.5µs to 3,824 B and 23.9µs**, which is an ordinary message's ingest ceiling on this path rising from about 26,000 to about 42,000 a second. A 25-entry market data snapshot went from 107,940 B and 172.9µs to 60,408 B and 89.5µs, and a 250-entry one from 884,408 B and 772.3µs to 562,400 B and 353.9µs. Measured by running the same benchmark on both sides of the change, with the superseded implementation kept alive in a test that asserts the two answers agree over nine message shapes, the venue's own undefined group and a malformed count row included.
- **Two things were measured and not kept, which is the point of measuring.** Memoising `isGroupTag` in a `ConcurrentHashMap<Int, Boolean>` looked free, since the new gate asks it once per body field. It was a regression: boxing the tag and allocating `computeIfAbsent`'s lambda per call cost **3.2x the allocation and 1.6x the time** of simply calling the function, so it is documented in the benchmark rather than shipped. The acceptor's reply path was then asked the same question, should a rule's reply be a prototype rather than a parse per reply, and the answer is also no: the parse is 3,786 B of a 15,576 B reply, one dispatch thread already holds about 34,000 replies a second, and the load runner's largest shape asks for 4,000. The number to attack there, if it ever matters, is the resolve chain and not the parse.

### 🔧 Changed

- **Setup and teardown steps are rows in the scenario editor.** They were a read-only line in the header (*setup: 1 step before the flow, wipes the session log each run*), so the only way to stop a captured scenario erasing the session's message log on every run was to open the JSON and delete the `clearMessages` step by hand. They are rows in the step list now, above and below the flow, badged **SETUP** and **TEARDOWN**, and they take the controls the list already had: mute, remove, re-order and the session picker. Re-ordering stays inside a phase, because moving a step across one of those boundaries changes when it runs rather than where it sits, and the flow keeps its own numbering, so *Step 3* is still the run report's step 3 however many setup rows sit above it. The header's summary is read off the rows, so muting the Clear step retires its warning in the same frame, and its ⓘ now says which control to reach for instead of leaving the reader to guess. **BINDING**'s two tooltips are rewritten for somebody who has never met the word *bind*: they say what an Expect step may match, and that THIS RUN does not clear the log.

- **A step says which of the three lists it runs in.** A row could be muted, removed and re-ordered but not re-filed, so a scenario written without a setup still could not gain one: Insert takes the selected row's phase, and with no setup row there was nothing to select. **runs in** sits beside the session in the step's detail pane, offers *setup*, *flow* and *teardown* with the sentence the runner would say about each, and lands the row at the end of the phase it chose, carrying its id, its mute state and whatever drafts are open on it. The badge, the numbering and the header's summary all follow it, because all three are read off the rows, and setting the picker back is the undo. The detail title's separator is a middle dot with it, so *Setup 1 · clear* reads like the row label *Expect 8 · 2 tags* it sits beside.

- **The bundled RFQ venue stops deriving anything.** Its QuoteID was `Q-` plus the QuoteReqID and its price was the pair's mid with no movement, both so a load run's second phase could address a quote it had never seen. That was the tooling showing through: an opaque QuoteID is the one thing every real RFQ venue has in common, and a firm price that never moves is the one thing none of them has, so a client written against the old venue would pass here and fail in the field. The QuoteID is now a uuid and each side is drawn from its own band, nine ticks wide on the pair's pipette, with the bands a full spread apart so **no draw can invert a quote**. What replaced the derivation is the venue's memory: three refusals sit above every other QuoteResponse rule (`297=9` for a quote it never sent, `297=7` for one that expired, `297=5` for one already answered), and the bookings compare the client's price against the quote's own and book at it. Seventeen rules where there were eighteen, because the symbol and the price come from the quote, so the three pairs are one buy rule and one sell rule. **A hit naming an instrument the quote did not is refused by name**, `297=5` with *Instrument is not the quoted one*, rather than falling into the price refusal and sending a client off to check pricing that was never at fault. The example ships `load-sets/rfq-round-trip.json`, whose first phase captures `quoteId` and `offer` and whose next two read them, which is what a client does. **A scenario or a load set written against the old ids needs the new shape**: read 117 and 133 back off the quote. The bundled *pass and counter* scenario is reordered for the same reason: a pass spends the quote, so everything after one is now refused as spent. **A copy you already opened keeps the old venue, and nothing tells you.** Opening an example is idempotent, by design, so a `rfq-venue` workspace opened before this release still holds the eighteen derived rules, the `Q-` QuoteID, the frozen prices and no `load-sets/` directory, and the release cannot reach into a folder that is yours. **Settings → Storage → Reset** is what lays the shipped example down again, and `load-sets/rfq-round-trip.json` arrives with that reset and not before. Reset renames your current copy to `rfq-venue-before-reset-<date>` beside the fresh one rather than deleting it, so an edited rule or a captured scenario is still there to be fetched, which is why it asks for no confirmation.
- **`fixtool load --set <k>=<v>` is now `--seed <k>=<v>`.** `--set` seeded a value on `load` while `fixtool run --set <name>` runs a saved set, so a load set had no spelling left. The seed flag is `--seed`, which is what the dialog calls the field and what the plan's own field is named, and `--set <name>` means the saved set on both commands. A `--set` whose value carries an `=` is still read as a seed **this release**, with one line on stderr saying to write `--seed`. Set names are slugs and cannot contain `=`, so the two never collide. The dialog's "Copy as fixtool load" line moves with it.
- **`--json` and `GET /loads/<id>` write the record, not the bare report.** Both wrote the report while the file on disk was already the record, and a set has no single report to write. All three doors now write `loads/<id>/load.json`'s shape and agree byte for byte. **This breaks a reader of `--json`'s top-level `.verdict`**, which is `.phases[0].verdict` from here, or the set-level `.exitCode`. It is the cheapest this will ever be: one release has shipped the bare shape, and the file itself already changed.
- **The record's lifecycle key is `stage`, and the schema is 3.** `LoadPhase` was the run's own lifecycle (preparing, issuing, settling, done) written as `"phase"`, while the record's array of set members is `phases[]`, so `phases[1].phase` was a sentence nobody should have to read. The lifecycle is `stage` and a set member is a phase, which is what load testers call one. The reader still accepts `"phase"`, and `GET /loads` rows say `stage`. Schema 3 also adds the SKIPPED and PENDING phase statuses and names each phase's evidence files inside the phase (`02-unmatched.fix`), so nothing downstream guesses a file name once a set writes several phases into one directory.

### 🐛 Fixed

- **A saved load set or a Load run started from the window now runs on a profile that opens one session.** The window gathered a load run's lanes through the fan-out check, which refuses a profile whose Sessions is 1 because a fan-out with one lane is not a fan-out. A load run is the other case: it issues without waiting for replies, and a real venue's load accounts are one session each, because the venue fans every reply out to every session of the organisation, so a second lane buys duplicates rather than throughput. `fixtool load --set` has always issued from that profile, so the same set ran from the command line and was refused from the window, in a sentence about the Sessions field and a `{nn}` pattern that would have made the run worse. The window now asks the load run's own question: the Run menu's load rows count a one-session profile among their lanes, the Load run dialog offers it, and a profile with nothing connected is told what is really missing, *no session of 'LOADGEN' is logged on. Connect it, then run the load.* **Fan out over sessions** is untouched, and still names the field and the pattern that fix it.
- **A profile that opens one session now resolves its `{n}` numbering, as slot 1.** The identity fields took a `{n}` or `{nn}` pattern so a group of lanes could each log on as themselves, and the resolver returned the config untouched whenever Sessions was 1, on the reasoning that one lane has nobody to be distinguished from. A venue hands out load-test accounts one session at a time, so `initiator_org_1_user_{n}@load.test` on a one-session profile is an ordinary identity, and it went to the wire as that literal text and was refused with *no account for username*. The only way through was to type the number in and lose the pattern. The numbering is now resolved at every session count, slot 1 being the first lane of a group of one, in the editor's preview as well as at logon and on reconnect. Comma-separated lists and the auto-generated SessionQualifier stay where they were, behind Sessions greater than 1, because they exist to tell several lanes apart and splitting on a comma at one session would cut short a password that happens to contain one.
- **A load set opens every lane of a profile that issues, before phase 1 dials, numbered from 1.** A set opens its lanes once and holds them for every phase, and it walked the phases in order taking whichever door each phase named. A two-sided set names both doors: the initiator issues in phase 1 while the responder listens, and the responder issues in phase 2 while the initiator listens. So the responder was first seen as a listener, which is one session, and phase 2 then issued from that single session whatever its profile said. Two things went wrong at once on a four-session profile: the phase ran on one lane instead of four, and because a listening session is not a numbered lane, `${sessionIndex}` rendered 0 on every message it sent, naming a user that does not exist. Every profile any phase issues from is now opened as lanes first, all of them, with the 1-based slots every other door hands out, and only a profile no phase ever issues from is opened as a listener. A profile that does both listens on its issuing lanes, which costs it nothing, because the set already puts one listener on every session it holds for the whole run.
- **A load run started from the window numbers a one-session profile's lane 1.** A run's lanes are built from the sessions the profile has up, and each lane takes the slot its session carries. A profile that opens one session carries slot 0 as a convention of its own, so `${sessionIndex}` rendered 0 on every message the window issued, while the same set run headless rendered 1 for that same session: a load client logged on as `RFQLG1` and sent `58=LANE-0` on the very same message. A group of one is lane 1 now, so the window and the command line answer the same number and the record's per-lane rows agree with what the messages actually carry.
- **"discarded on N sessions" now counts the sessions the number was taken over.** The line worked it out as the lane count plus the number of `--listen` profiles named, which reads a profile as one session. A listening profile that opens three was three sessions short, one that never reached LOGGED_ON was one over, and a profile that issues on the lanes it also listens on was one session counted twice, which is the two-sided set this release adds. The run has always known the answer, because the discard delta is summed over exactly that list of sessions with the repeats removed, so the count is carried in the record beside the delta and the line reads it. A record written before it says "0 discarded" and claims nothing about sessions rather than guessing.

## [1.17.0] - 2026-09-07

The release where the tool stopped waiting for an answer before sending the next message. Fan out
runs one scenario on N lanes a step at a time, so messages in flight were capped at the lane count
and there was no way to say "500 a second for ten minutes". A load run issues at a rate it never
skips, accounts for every reply that lands on any participating session, and reports what the tool
itself failed to issue beside what the venue failed to answer — from the command line, from the
window and from the control surface, over profiles that can now keep their messages in memory
rather than on disk. And a load run needs something to load: the RFQ Venue is a second bundled
example, a request-for-quote desk that answers everything a client can say, built to be run against.
Which is how the first load run against it found the Kotlin compile behind every shorthand
timestamp in a reply.

### ✨ Added

#### Load runs: issue at a rate, and account for every reply

- **`fixtool load`: a load run from the command line.** Fan-out runs one scenario on N lanes, one step at a time, so messages in flight were capped at the lane count and there was no way to say "500 a second for ten minutes". `fixtool load <template> --profile LOADGEN --count 4000` issues a saved message across a profile's sessions **without waiting for replies**, as a burst or at a rate the run never skips, then accounts for every reply that lands on any participating session, including a `--listen` drop-copy. `${messageIndex}`, `--set` seeds, `${uuid}` and `${utcnow}` vary per message; everything else is evaluated once per lane and the report says which. The report has three verdicts and one exit code: completeness, rate (a shortfall exits 0 unless `--strict-rate`), and tool, because a harness that quietly under-issues reports the venue as faster than it is, so issued is three numbers and the tool's own discards and refusals are in the report. The record under `loads/<id>/` is `load.json`, the wire of every unanswered request and fifty specimen pairs, never every message. `--json` and `--junit` write it where a build wants it.
- **Load runs from the app.** The Message Editor gains a **⚡ Load** button beside Send and Send to All, and the Scenarios rail's Run menu gains **Load run…** under Fan out over sessions. Both open one dialog, one row per decision, where every refusal is a sentence on screen and Run is what refuses. The run opens as a document tab that updates live, with Stop on its header, and reads the same record back when **Recent ▸** reopens it later, so a run started from the terminal overnight is the same document in the morning. A lane whose store or log differs from what the run asks for is reconnected for the run and reconnected back after it.
- **Load runs from the control surface.** `POST /load` starts one as a job and answers 202 with its id, `GET /loads` lists recent runs, `GET /loads/<id>?wait=` polls one and returns the same report `fixtool load --json` writes, and `POST /loads/<id>/stop` stops it. MCP gains `fixtool_load` and `fixtool_load_status`, the same shape as the run set tools.

#### A profile chooses where its messages are kept

- **A profile chooses its message store and its message log.** Both used to be QuickFIX/J file factories unconditionally, which is the right default for interactive use and the wrong one for a load or soak run: every message in both directions was appended to the store and to the per-session log, so the run's length was bounded by disk and a lane's issue rate by file I/O. Under **Advanced Settings** a profile now picks **Message store: File or Memory** and **Message log: File or None**. The defaults are what they always were. A memory store needs Reset on Logon, since it starts at 1 on every logon, and the panel, `POST /profiles` and `/connect` all say so in the same sentence rather than flipping the flag behind your back. An acceptor profile's choice applies to every client session it creates.

#### A second example: the RFQ Venue

- **A second example: the RFQ Venue.** Open workspace offers an **RFQ Venue** beside the FX Venue: a request-for-quote desk on `localhost:19877` that a client drives through the whole flow, QuoteRequest in, a firm two-way Quote out, QuoteResponse in, ExecutionReport out for the booked trade, and a QuoteStatusReport for everything that cannot be booked, a counter, a pass, a hit at the wrong price, a missing ClOrdID or size, an unpriced symbol, so nothing a client can say is answered with silence. Two things differ from the FX venue on purpose and the help says why: the QuoteID follows from the QuoteReqID (`131=RFQ-7` is quoted as `117=Q-RFQ-7`), and the prices are firm, so a client can address a quote it never saw and a rule can refuse a hit at any other price. The shapes follow the bundled FIX 4.4 dictionary rather than memory of it, and every reply the bundle can plan is validated against it in the tests. The example ships two clients, a third **RFQ Load Client** of five sessions on a memory store with no message log so the first load run needs no profile edits, templates that lift, hit, counter or pass the last quote, two load templates that request and then book the same quotes from one `run` seed, and two scenarios that run green twice. The rule editor's Bundles group offers the venue's eighteen rules for an acceptor of your own. The empty session area now offers one button per bundled example, `POST /workspace {"example":"rfq-venue"}` opens it from a script, and `POST /demo` stays the FX venue.
...
The release where the demo stopped being something installed into your own files and became a
workspace you open, name and keep. A workspace is a folder holding its own profiles, saved messages,
scenarios and session store; the FX venue is one we ship inside the application, and Open copies it
out to a folder that is yours. Passwords left the file you would share, environments split where a
counterparty is from who it is, and the venue — being the container its clients live in rather than
a peer of them — gave its grid cell back.

### ✨ Added

#### Workspaces: the demo becomes an example you open, name and keep

- **Workspaces: New, Open, Recent, Close — and the demo is gone.** Start Demo Server built three profiles, eleven templates and two scenarios in Kotlin, installed them into **your own** profiles list and scenarios directory, and Stop hunted them back down by id prefix. There is no demo now: a **workspace** is a folder holding its own profiles, saved messages, scenarios and session store, and the FX venue is a bundled example that **Open** can open like any other. Opening it copies it to `~/.fixtool/workspaces/fx-venue` and opens that, because the bundle lives inside the application and cannot be edited in place — and the copy happens **once**: opening the example again returns to the workspace you already have, with your edits in it, rather than minting a numbered clone beside it. A copy remembers which example it came from, in a small `.fixtool-origin` file, so **Settings → Storage** can offer **Reset**: the shipped example laid down again in the same folder, with your current copy *renamed* to `fx-venue-before-reset-<date>` rather than deleted. Nothing is destroyed, so Reset asks nothing. The origin file is also the only thing that distinguishes a copy from a workspace you made yourself, which matters because `New workspace… → "FX Venue"` lands in exactly the same place. **New workspace…** asks for a name and a location and makes an empty one. Nothing is mixed into your files, so there is nothing to uninstall: **Close workspace** puts your own profiles and scenarios back and leaves the copy on disk, because it is yours. All of it is on a **switcher at the top left** showing the open workspace by name — not in Quick Connect, which connects a profile and has no business changing which profiles exist. Recent lists workspaces by their own path rather than the folder that holds them, and Default never appears there because Close is what returns you to it.
...
#### Environments: a connection is a counterparty times an environment
...
- **Minimize a pane.** Split view had two ways to quiet a pane and both were wrong: close, which disconnects the session and throws its log away, and switching to tabs, which abandons the side-by-side you were in split view for. There was no way to say *not now*. The **—** button on any pane now sends it to a strip above the grid, where its chip brings it back. The session keeps running, keeps logging, and keeps being the message editor's send target — a chip marked `→ editor` is where **Send** will go, because minimizing a pane must never quietly point a composed order at a different counterparty. `GET /sessions` reports `minimized` on a pane that is in the strip.
- Minimizing takes a pane *out of the grid* rather than collapsing it in place, and that is what makes the space come back: a cell is sized from the row and column weights and never from its content, so a pane collapsed to its header would be a header floating in a full-size cell. Removed from the list, the existing column arithmetic reflows on its own. The decision is remembered per profile across restarts.

### 🔧 Changed

#### A venue is a container, not a peer of its own clients

- **A venue is not a session, so it no longer gets a session's pane.** An acceptor with `TargetCompID = *` was laid out as a peer of its own clients, with a full grid cell and the full session toolbar, and it is neither: it is the container those clients live in, and its own pane holds no traffic at all — every message on a venue belongs to one of its counterparties. It now starts **minimized**, as a chip carrying the port, who is connected, and its controls. On the shipped FX Venue example this is the difference between five panes that the grid rounds up to six cells with one left blank, and a clean 2×2.
- **The venue pane's toolbar was eleven controls of which four did anything.** Filter, group-by-conversation, add blank line, clear messages, scroll-to-bottom, wrap and search all act on a message grid that a venue does not have. The filter was the worst of them: its bar drew *above* the venue check, so it rendered a working-looking regex box over the client list and filtered nothing. They are gone from a venue at both sizes, in split view and in tabs.
- **A venue's rules are reachable from the venue.** Rules and latency live in the profile editor, and the pane representing the venue only echoed a count of them — there was no route from one to the other. Both the pane and the chip now carry a named **Rules** button that opens the profile's auto-responses, unfolding the section even when there are none, which is the likeliest reason to have pressed it.
- **Stop and Start replace the bare power icon on a venue.** On a venue that icon unbinds a port every client on the list is sitting on, and said none of that. **Clear refused** is its own button too: *Clear all messages* never touched the refusal log, so on a venue it was both the wrong promise and a no-op on an already-empty list.
- **A venue says nothing while it is healthy.** Its chip and its pane derive everything from one description of the venue, so the chip can only ever be a subset of the pane and neither can learn a fact the other cannot say. What earns a place is identity and live state always, everything else only when it deviates: a client gone, a logon refused, replies pending, latency on, *no* rules loaded, or replies triggered and not sent. Twenty-one rules loaded is furniture; zero rules is the answer to why the venue is silent. So a healthy venue's chip is the short one, and it widens in proportion to how much has gone wrong.
- **Clicking a client on a venue's overview always focused that client's pane, and never looked like it could.** A bare row with no hover state, no cursor change and no chevron is an action nobody finds. It has all three now. The move-left and move-right arrows became icons, too — they were the only text glyphs in a row of icons and so the only buttons with no accessible name.
- Panes are addressed by session rather than by index wherever the UI asks for one to be closed, moved or focused. With a strip in play, "one to the left" means one *visible* pane to the left, and an index computed from the grid counts a shorter list than the model's — which does not fail loudly, it moves the wrong pane, or closes one and disconnects it.

#### Browse and Save use the platform's own dialog
...
#### The demo is a workspace action, and the control surface follows
...
- **Two MCP tools stopped describing themselves wrongly.** `fixtool_demo` advertised `name` and `fixVersion` and read neither, so a caller asking for FIX 4.2 got a 4.4 session and no warning — the same theatre the New workspace dialog dropped this release, and for the same reason: a loaded data dictionary decides the wire version at connect time. Both arguments are gone; `fixtool_workspace` is the route to opening the example under another name or somewhere else. `fixtool_delete_profile` and `DELETE /profiles` also stop saying demo profiles are protected, which was never true of any profile — the endpoint has always passed the id straight through.
