...
**A FIX test workbench.** Run either end of a FIX session, simulate the venue on the other side, and prove what passes between them: build and send messages, answer them with rules rather than code, capture an exchange into a scenario that asserts every reply, push thousands of messages through a load run, and follow one exchange across every session it touched.
...
- **Workspaces**: A workspace is a folder holding its own profiles, saved messages, scenarios and session store — commit one beside the code it tests, or hand it to a colleague. Logon passwords stay out of the shareable file. A workspace can name the FIX dictionary its scenarios were written in, and that one is loaded over Settings while it is open. Five bundled examples — an **FX Venue**, an **FX RFQ Venue**, an **Equity Venue**, a **Crypto Venue** and a **Fixed Income RFQ Platform** — open as workspaces of your own, each with scenarios and a load set to run against it
