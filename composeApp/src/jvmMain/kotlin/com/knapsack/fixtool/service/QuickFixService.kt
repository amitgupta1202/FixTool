...
class QuickFixService(
    ...
    private val relayVenue = LiveRelayVenue(rfqBook, counterparties = { config.counterparties })
    ...
    private fun noteNotDelivered(sessionId: SessionID, ruleIndex: Int?) {
        ...
        return AcceptorStatus(
            ...
            pendingResponses =
                if (isVenue) {
                    ...
                    (channels.keys + autoResponseDispatch.sessionsWithPending())
                        .sumOf { autoResponseDispatch.pendingCount(it) }
                } else {
    ...
    override fun onLogout(sessionId: SessionID) {
        ...
        autoResponseDispatch.cancelAll(sessionId).forEach { reason -> noteNotDelivered(sessionId, reason?.ruleIndex) }
    ...
    private fun book(sessionId: SessionID, fixMessage: FixMessage, message: Message, sent: Boolean) {
        ...
        try {
            ...
            if (config.counterparties.isNotEmpty() && relayVenue.record(sessionId, fixMessage, message, sent)) return
    ...
    override fun fromApp(message: Message, sessionId: SessionID) {
        ...
        try {
            ...
            val relaying = config.counterparties.isNotEmpty()
            val venueBefore = if (relaying) relayVenue.reading(sessionId, parsedMessage) else null
     ...
     *
     * Three readings rather than one bundle, because each is taken of a different book at the moment decision 4a
     * names, and the call site is where a reader checks that they were taken before the message was recorded.
     */
