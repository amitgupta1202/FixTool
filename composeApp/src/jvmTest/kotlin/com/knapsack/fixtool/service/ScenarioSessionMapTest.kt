...
/**
 * [withSessions] is the "same flow, other environment" primitive — behind both the materialized
 * environment copy and the control surface's throwaway remap. The tests pin its two promises: every
 * phase's sessions go through the map, and nothing else about the steps (identity above all) moves.
 */
class ScenarioSessionMapTest {
    ...
    @Test
    fun `step identity survives a remap — a run of the copy reports the same step ids`() {
