...
| `POST /scenarios/run` | `{"id"}` or `{"scenario":{…}}`, `format`?, `sessions`?, `mapping`? | run a scenario deterministically → per-step/per-tag report (or JUnit XML with `format:"junit"`). `sessions` is an inline `{from: to}` session remap for this run only; `mapping` names a saved session mapping — either re-aims the run (e.g. at the QA pair) without editing the scenario. Sessions the run needs are auto-connected from saved profiles |
