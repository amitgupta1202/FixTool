...
| `POST /panel`        | `{"panel":"connection\|editor\|detail\|settings\|scenarios\|conversations\|pane\|trace\|orderbook\|latency\|terminal", "show"?, "follow"?, "render"?, "profile"?, "rule"?, "step"?, "action"?}` | show/hide a panel (`latency` and `terminal` are those two tool windows; `scenarios` toggles the Scenarios rail; `conversations` sets group-by-conversation — per session with `"session"`, all sessions without; `pane` with a `"session"` minimizes that pane to the strip above the grid (`show:false`) or puts it back (`show:true`), which is how a venue's own pane, minimized from the start, puts its counterparties and RFQ book on screen; `trace` opens the Trace panel and takes `"follow"`: a whole correlation value to narrow every pane to that exchange, or `null` to stop, and `"render"`: `ledger` (the grid, every trace) or `lanes` (the followed trace as swimlanes) — see [Following one exchange across every session](#following-one-exchange-across-every-session); `connection` takes `"profile"` to load that profile onto the form, as clicking it in the list does; `editor` with a `profile` and a `rule` (+ optional `step`) opens that acceptor rule's reply step in the message editor, and `action:apply`/`action:cancel` finishes it) |
...
| `POST /load`         | `{"set", "seed"?, "onFailure"?}` or `{"phases":[…], "seed"?, "storeAndLog"?, "onFailure"?}` | **starts a load set as a job** → 202 `{load, status, label, phases}`; 404 when no set answers to the name; `{error, problems:[…]}` when a plan is wrong, `error` being the first sentence and `problems` every one of them, each naming its phase. With `set`, `seed` overrides the file's seed. Inline, `seed` **is** the set's seed and its generators are rendered once at set start. See [Load sets](#load-sets---set-name) |
| `GET /loads`         | —                                      | recent load runs, newest first: `{count, loads:[{id, label, status, stage, phases:{total, done, current?}, issued, matched, unmatched, startedAt, finishedAt?, exitCode?}]}`. The counts are the live phase's, else the phase the verdict names, never a skipped one's zeroes. `exitCode` is absent while any phase is still to come, so a row that carries one is over |
...
| `GET /run`           | —                                      | **the run configuration widget's selection**: `{selected:{kind:"loadSet"\|"runSet", name, label?}\|null, running, load\|runSet?, refusal?}` — what the toolbar's ▶ is pointed at, the live id while that runs, and why ▶ would be refused. Reads through the widget's own rules, so it answers what the chip shows |
| `POST /run`          | `{}`, `{"loadSet"\|"runSet": "<name>", "start"?}` or `{"stop": true}` | **run what ▶ names.** `{}` runs the selection through `POST /load {"set"}` or `POST /scenarios/run {"set"}` and answers with that door's code and body — 202 and the job id, 409 while another run holds the sessions, a set's own `{error, problems}` — plus `configuration`. `loadSet`/`runSet` aims the widget first, as choosing a row in its menu does, and the choice is remembered; `"start": false` aims without running. `{"stop": true}` stops the selection's own live run → 202, or 409 when it is not running. 404 when nothing is saved or the name is not a saved set. MCP: `fixtool_run_selection` with `action` `status` (default), `aim`, `run` or `stop` |
| `GET /load-sets`     | —                                      | the saved load sets: `{count, sets:[{name, label, phases, muted?, profiles}]}`. `muted` is how many of the phases are parked, present only when any is |
...
| Run › Run / Stop what the run widget names | ⌃R | `POST /run`, `POST /run {"stop":true}`, `GET /run` |
| Run › Load run… | — | `POST /load` |
| Run › Load set ▸, Run set ▸ | — | `POST /run {"loadSet"\|"runSet", "start":false}` chooses one; `POST /load {"set"}`, `POST /scenarios/run {"set"}` start one by name |
