...
> Phase 3 (Run): the full in-app authoring UX — the Scenarios panel (run / list / delete), the
> per-tag **red/green run-results rendering**, the **visual scenario builder** (per-step
> multi-session), and the **expectation builder** with editable matcher chips, live preview,
> dictionary auto-seed, group-by-identity, and the two-instance verify-generalizes check —
> [`repeatable-scenarios-phase3-evidence.md`](./repeatable-scenarios-phase3-evidence.md). Nothing deferred.
...
> "Automated Verification (Assertions)" section. Evidence and the file-by-file map:
> [`repeatable-scenarios-phase1-evidence.md`](./repeatable-scenarios-phase1-evidence.md).
...
## Phase 3 — Run: in-app authoring UX ✅ Delivered (complete — nothing deferred)

**Goal:** a QA engineer (or agent) authors and saves scenarios in the app, without
hand-editing JSON.

> **Delivered, in full.** All of 3.1–3.6:
> - **3.5** per-tag red/green **run-results overlay** (`ScenarioResultsView`) — the in-app
>   rendering of a failed assertion.
> - **Scenarios panel** (toolbar ▶ button → `ScenariosDialog`): run / list / delete, author by
>   pasting JSON, or **Build scenario** visually. Runner shared with the control surface via
>   `ViewModelScenarioHost`.
> - **3.4 / 3.6** the **visual `ScenarioBuilder`**: add Send/Wait/Expect/Clear/Reset steps by
>   clicking, **per-step session** selector (multi-session authoring), reorder/remove, save.
> - **3.1 / 3.2 / 3.3** the **`ExpectationBuilder`** + **`MatcherEditor`**: a captured message
>   rendered tag-by-tag with editable matcher **chips**, dictionary **auto-seeding** (capture),
